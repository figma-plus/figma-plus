<template lang="pug">
	.detail-screen
		.header
			.back-button(@click='backToList') Ŕ	
			.title
				.name {{ plugin.name }}
		...
		.content
			.section.image-section(v-if='plugin.images && plugin.images.length > 0' :class='{singleImage: plugin.images.length === 1}')
					...
					.v-carousel-nav.prev(@click="$refs.carousel.$emit('prev')") Ŕ
					.v-carousel-nav.next(@click="$refs.carousel.$emit('next')") ŕ
...
export default {
  ...
  computed: {
    latestUpdate() {
      if (this.plugin.updates && this.plugin.updates.length > 0) {
